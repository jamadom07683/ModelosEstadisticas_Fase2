...
# Filtrar datos donde P1010 (ingresos) sea menor a 100 millones sin PW
datamall <- variables_seleccionadas %>%
  filter(P1010 < 300000000  & !is.na(P1010))

summary(datamall$P1010)

head(datamall)
print(nrow(datamall))

# Create the histogram with ggplot2
ggplot(datamall, aes(x = P1010)) +
  geom_histogram(aes(y = ..density..), bins = 30, fill = "lightblue", color = "black") +
  labs(title = "Histograma de Ingresos Menores a 300 millones",
       x = "Ingresos",
       y = "Densidad") +
  
  # agregar una funcion normal
  stat_function(fun = dnorm, args = list(mean = mean(datamall$P1010, na.rm = TRUE),
                                          sd = sd(datamall$P1010, na.rm = TRUE)),
                color = "red", size = 1) +
  
  ### agregar una funcion de distribucion lognormal
  stat_function(fun = dlnorm, args = list(meanlog = log(mean(datamall$P1010, na.rm = TRUE)),
                                           sdlog = sd(log(datamall$P1010), na.rm = TRUE)),
                color = "blue", size = 1) +
  theme_minimal()

  meanlog = log(mean(datamall$P1010, na.rm = TRUE))
  sdlog = sd(log(datamall$P1010), na.rm = TRUE)                
...
```
Para calcular la probabilidad de que los ingresos sean mayores de 100 millones, dado que siguen una distribución lognormal tenemos que:

Transformar el límite de ingresos: Debemos convertir 100 millones a la escala logarítmica usando la función de logaritmo natural (⁡ln).
    x=ln⁡(100,000,000)≈18.4207

Calcular la probabilidad: Usamos la media (μ) y la desviación estándar (σ) de la distribución lognormal encontrada arriba, que son \[ \mu=17.9433\] y  \[\sigma=1.14008\]


La probabilidad de que los ingresos sean mayores de 100 millones, dado que siguen una distribución lognormal con un logmean de 17.9433 y un sdlog de 1.14008, es aproximadamente 0.3377, o 33.77%. 
