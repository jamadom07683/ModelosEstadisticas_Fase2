...
# Filtrar datos válidos para P1010 (ventas)
ventas_validas <- datos_microestablecimientos %>% filter(!is.na(P1010))

# Calcular media, desviación estándar y tamaño de muestra
...
# Calcular el intervalo de confianza (IC) al 95%
z_score <- qnorm(0.975)  # Valor Z para un IC del 95%
...
```
## Calcular el intervalo de confianza para la proporción de microestablecimientos que cuentan con página web (P2532)
```{r echo=FALSE, message=FALSE, warning=FALSE}

# Filtrar datos válidos para P2532 (página web)
pagina_web_valida <- datos_microestablecimientos %>% filter(!is.na(P2532))

# Calcular proporción de establecimientos con página web
proporcion_pagina_web <- mean(pagina_web_valida$P2532)

# Calcular error estándar para una proporción
...
# Filtrar datos válidos para P2524 (acceso a Internet)
internet_valido <- datos_microestablecimientos %>% filter(!is.na(P2524))

# Calcular proporción de establecimientos con acceso a Internet
...
# Filtrar datos válidos para P1088 (número de portátiles)
...
# Referencias.
```{r echo=FALSE, message=FALSE, warning=FALSE}
cat("Los datos utilizados para este análisis fueron obtenidos de la base de datos de microestablecimientos del DANE (2016). 
